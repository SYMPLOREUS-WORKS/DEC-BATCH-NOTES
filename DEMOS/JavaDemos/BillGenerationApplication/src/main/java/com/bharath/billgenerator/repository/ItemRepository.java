...
//	@Query(value="update item_details set category = ?1 where name= ?2", nativeQuery = true)
	...
	//@Query(value = "update item_details set price=:price where name=:name", nativeQuery = true)
	
	@Query(value = "update Item set price=:price where itemName=:"
			+ "", nativeQuery = false)
	@Modifying
	@Transactional
	void updatePriceOfItem(@Param(value = "nameRandom") String name1, @Param(value = "price") Double price1);
}
